...
def listen_main_server():
    ...
    sock.bind(('0.0.0.0', 33418))  # связываем сокет с портом, где он будет ожидать сообщения
...
async def working_sensor():
    while True:
        try:
            if flag == '/on':
                print('<SENSOR> w_1 ' + str(random.randint(15, 25)) + '°C')
            elif flag == '/off':
                print('<SENSOR> w_1 Я сплю!')
            else:
                print('<SENSOR> w_1 Unknown command!')
