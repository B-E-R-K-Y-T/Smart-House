...
def listen_main_server():
    ...
    sock.bind(('0.0.0.0', 14507))  # связываем сокет с портом, где он будет ожидать сообщения
...
async def working_sensor():
    while True:
        try:
            if flag == '/on':
                print('<SENSOR> q_1 ' + str(random.randint(15, 25)) + '°C')
            elif flag == '/off':
                print('<SENSOR> q_1 Я сплю!')
            else:
                print('<SENSOR> q_1 Unknown command!')
